...
describe UsersController do
  ...
  def mock_arguments(merge = {})
    args = merge
    args.merge!(:current_user= => nil, :reset_password => nil, :password => "pass", :email => "email", :login=> "login", :attributes= => nil, :update_all_children => nil, :errors => {})
  ...
  before(:each) do
    ...
    mock_configuration = mock_model(CONFIGURATION, {:password_sender_email => "email", :login_url => "url"})
  ...
  describe "PUT udpate" do
    ...
    describe "with valid params" do
      ...
      it "exposes the requested user as @user" do
        User.stub!(:get!).and_return(mock_user(mock_arguments(:save => true)))
        put :update, :id => "1"
        assigns(:user).should equal(mock_user)
      end
    ...
    describe "with invalid params" do
      ...
      it "exposes the user as @user" do
        User.stub!(:get!).and_return(mock_user(mock_arguments(:save => false)))
        mock_user.should_receive(:dirty?)
        put :update, :id => "1"
        assigns(:user).should equal(mock_user)
      end
      ...
      it "re-renders the 'edit' template" do
        User.stub!(:get!).and_return(mock_user(mock_arguments(:save => false, :dirty? => true)))
        put :update, :id => "1"
